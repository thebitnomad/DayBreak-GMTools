...
//				#{BtnDetectorConfig}.setText( msgOK );
			...
			var updategame = function( ) {
				...
				var pakName = getFileName( pakFilePath );
				if( pakName.length == 0 ) {
...
//			};
			...
			var configGameProc = function( ) {
				...
				win.load( {
					url: 'svrcfg.aspx',
					params: { procId: selGameGroup.get( 'ProcId' ) },
					discardUrl: false,
					nocache: true,
					timeout: 30
				} );
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentHolder" Runat="Server">
					...
					<ext:GridPanel ID="GridPanelGameGroup" runat="Server" Border="false" StoreID="StoreGameGroup" AutoExpandColumn="Name" > <%--ColumnLines="true"--%>
						...
						<ColumnModel>
							<Columns>
								...
								<ext:Column Header="<%$ Resources:StringDef, Type %>" DataIndex="DisplayFlag" Align="Center" Width="50" >
									<Renderer Fn="renderDisplayFlag" />
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef, Detector %>" DataIndex="NetStatus" Width="60" Align="Center" Sortable="false" >
									<Renderer Fn="renderGameGroupNetStatus" />
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef, Game %>" DataIndex="GameStatus" Width="60" ColumnID="GameStatus" Align="Center" Sortable="false">
									<Renderer Fn="renderGameGroupGameStatus" />
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef, Payment %>" DataIndex="PaymentStatus" Width="60" Align="Center" Sortable="false">
									<Renderer Fn="renderGameGroupPaymentStatus" />
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef, Database %>" DataIndex="DBStatus" Width="60" Align="Center" Sortable="false">
									<Renderer Fn="renderGameGroupDBStatus" />
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef, NetErrStatus %>" DataIndex="NetErrStatus" Width="60" Align="Center" Sortable="false">
									<Renderer Fn="renderGameGroupNetErrStatus" />
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef, PlayerCount %>" DataIndex="PlayerCount" Width="60" Align="Center">
								...
								<ext:CommandColumn Width="370">
									...
									<Commands>
										<ext:GridCommand CommandName="StartGame" Text="<%$ Resources:StringDef, StartGame %>" Icon="PlayGreen" />
										<ext:GridCommand CommandName="StopGame" Text="<%$ Resources:StringDef, StopGame %>" Icon="StopBlue" />
										<ext:GridCommand CommandName="UpdateGame" Text="<%$ Resources:StringDef, UpdateGame %>" Icon="Build" />
										<ext:GridCommand CommandName="SvrPakUpload" Text="<%$ Resources:StringDef, Upload %>" Icon="FolderGo" />
										...
										<ext:GridCommand CommandName="EventHis" Text="<%$ Resources:StringDef, Info %>" Icon="CarStart" />
									</Commands>
	...
	<ext:Window ID="WindowGameGroup" runat="server" Resizable="false" Width="600"
		...
		<Items>
			<ext:FormPanel ID="FormPanelGameGroup" runat="server" ButtonAlign="Right" Padding="10" 
				...
				<Items>
					<ext:TextField ID="TextFieldGameGroupId" runat="server" Width="450" FieldLabel="<%$ Resources:StringDef, Id %>" Disabled="true" >
					</ext:TextField>
					<ext:TextField ID="TextFieldGameGroupName" runat="server" Width="450" FieldLabel="<%$ Resources:StringDef, SvrName %>">
					</ext:TextField>
					<ext:SpinnerField ID="TextFieldGameGroupIndex" runat="server" Width="450" FieldLabel="<%$ Resources:StringDef, SerNum %>" Note="<%$ Resources:StringDef, MsgSerNumNote %>">
					</ext:SpinnerField>
					...
					<ext:TextField ID="TextFieldGameGroupComment" runat="server" Width="450" FieldLabel="<%$ Resources:StringDef, Comment %>">
					</ext:TextField>
					<ext:CompositeField runat="Server" FieldLabel="<%$ Resources:StringDef, Flag %>" Width="450" >
						<Items>
					...
					<ext:CompositeField runat="Server" FieldLabel="<%$ Resources:StringDef, Type %>" Width="450" >
						<Items>
					...
					<ext:MultiCombo ID="MultiSelectRegion" StoreID="StoreRegion" FieldLabel="<%$ Resources:StringDef, BelongedRegion %>"
						runat="server" Width="450" ValueField="Id" DisplayField="Name" Mode="Local" >
					</ext:MultiCombo>
					<ext:CompositeField runat="Server" FieldLabel="<%$ Resources:StringDef, GameProcess %>" Width="450" >
						<Items>
							...
							<ext:ComboBox ID="ComboAddProcMachineId" StoreID="StoreMachine" runat="server" ValueField="Id"
								DisplayField="IpAddress" ForceSelection="true" Mode="Local" Width="120">
								<Listeners>
									<Select Fn="comboAddProcMachineSelect" />
								</Listeners>
							</ext:ComboBox>
							<ext:ComboBox ID="ComboAddProcId" StoreID="StoreQueryGameProc" runat="server" ValueField="Id"
								DisplayField="GameAppPath" ForceSelection="true" Mode="Local" Width="200">
							</ext:ComboBox>
						</Items>
					</ext:CompositeField>
				</Items>
	...
	<ext:Window ID="WindowBatchOp" runat="server" Resizable="true" Maximizable="true" Title="<%$ Resources:StringDef, BatchOperation %>"
		...
		Hidden="true" Modal="true">
		<AutoLoad Mode="IFrame" MaskMsg="<%$ Resources:StringDef, Loading %>" ShowMask="true" Url="svrctrl.aspx" >
		</AutoLoad>
	</ext:Window>
	...
	<ext:ToolTip ID="ToolTipGameGroup" runat="server" Target="#{GridPanelGameGroup}.getView( ).mainBody" TrackMouse="true" AutoHide="false"
		Delegate=".x-grid3-cell" Width="300" ShowDelay="500">
		<CustomConfig>
