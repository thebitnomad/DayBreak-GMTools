...
//		};
		...
		var fileManDownload = function( ) {
			...
			window.open( "../common/fileman.ashx?op=fdown&ult=" + m_fileUploadType + "&fpath=" + encodeURI( filePath ) );
		};
		...
		var fileManGetSelFile = function( ) {
			...
			if( record ) {
				return #{FieldRelDirPath}.getValue( ) + record.data.Name;
			} else {
				return '';
			}
		};
