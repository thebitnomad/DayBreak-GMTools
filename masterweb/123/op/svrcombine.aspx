<%@ page title="" language="C#" masterpagefile="~/common/main.master" autoeventwireup="true" inherits="op_svrcombine, App_Web_svrcombine.aspx.b81705c1" enableviewstate="true" theme="default" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeaderHolder" Runat="Server">
	...
	<ext:XScript ID="XScript1" runat="server">
		<script type="text/javascript">
			...
			var fixWarSvrData = function( ) {
				var result = showConfirmMsg( msgTitle, msgCommonOpConfirm, function( btn ) {
					if( btn == "yes" ) {
						Ext.net.DirectMethods.FixWarSvrData( {
							eventMask: {
								showMask: true,
								minDelay: 200,
								msg: msgSubmitting
					...
					success: function( result ) {
						if( result.Success ) {
							...
							if( !result.Result.AllFinish ) {
								setTimeout( "refreshImportState( );", 10000 );
							}
						}
						else if( result.ErrorMessage ) {
							showErrMsg( msgTitle, result.ErrorMessage );
			...
			var refreshExpoprtState = function(  ) {
				Ext.net.DirectMethods.RefreshExportState( {
					success: function( result ) {
						if( result.Success ) {
							#{DisplayFieldExportInfo}.setValue( result.Result.Desc );
							
							if( !result.Result.AllFinish ) {
								setTimeout( "refreshExpoprtState( );", 10000 );
							}
						}
						else if( result.ErrorMessage ) {
							showErrMsg( msgTitle, result.ErrorMessage );
			...
			var refreshBackupDataBaseState = function( ) {
				Ext.net.DirectMethods.RefreshBackupDataBaseState( {
					success: function( result ) {
						if( result.Success ) {
							#{DisplayFieldBKDataBaseInfo}.setValue( result.Result.Desc );
							
							if( !result.Result.AllFinish ) {
								setTimeout( "refreshBackupDataBaseState( );", 10000 );
							}
						}
						else if( result ) {
							showErrMsg( msgTitle, result.ErrorMessage );
			...
			var btnBeforeExport = function( ) {
				#{BeforeExport}.setVisible( true );
				Ext.net.DirectMethods.BeforeCombineAccount({
					success: function( result ) {
						if( result.Success ) {
							
							#{BeforeExport}.setValue( result.Result.Desc );
						}
						else if( result.ErrorMessage ) {
							showErrMsg( msgTitle, result.ErrorMessage );
						}
			...
			var btnAfterCombine = function( ) {
				#{AfterCombine}.setVisible( true );
				Ext.net.DirectMethods.AfterCombineAccount({
					success: function( result ) {
						if( result.Success ) {
							
							#{AfterCombine}.setValue( result.Result.Desc );
						}
						else if( result.ErrorMessage ) {
							showErrMsg( msgTitle, result.ErrorMessage );
						}
			...
			var svrWarOnKeyBoard = function( ) {
				#{SvrWarOnClose}.setVisible( true );
				Ext.net.DirectMethods.SvrWarOnSwitch({
					success: function( result ) {
						if( result.Success ) {
						
							#{SvrWarOnClose}.setValue( result.Result.Desc );
			...
			<ul class="user-info-list">
				...
				<li>
					...
					<ext:Panel ID="Panel5" runat="server" Layout="Form" LabelAlign="Left" LabelWidth="1" Border="false" Width="750" ButtonAlign="Center">
						<Content>
							...
							<ext:DisplayField ID="DisplayFieldBKData" runat="Server" Text="<br />"></ext:DisplayField>
							<ext:DisplayField ID="DisplayFieldBKDataBaseInfo" runat="Server"></ext:DisplayField>
						</Content>
						<Buttons>
				...
				<li>
					...
					<ext:Panel ID="Panel6" runat="server" Layout="Form" Border="false" Width="750" LabelWidth="1" ButtonAlign="Center">
						<Items>
							...
							<ext:DisplayField ID="DisplayFieldLineExport" runat="Server" Text="<br />"></ext:DisplayField>
							<ext:DisplayField ID="DisplayFieldExportInfo" runat="Server"></ext:DisplayField>
						</Items>
						<Buttons>
				...
				<li>
					...
					<ext:Panel ID="Panel3" runat="server" Layout="Form" Border="false" Width="750" LabelWidth="1" ButtonAlign="Center">
						<Items>
							<ext:DisplayField ID="DisplayFieldCombineMsg" Text="<%$ Resources:StringDef, MsgAccountCombineNote %>" runat="Server" >
							</ext:DisplayField>
							<ext:DisplayField ID="BeforeExport" runat="Server" Hidden="true" >
							</ext:DisplayField>
							<ext:DisplayField ID="AfterCombine" runat="Server" Hidden="true">
							</ext:DisplayField>
						</Items>
						<Buttons>
						    <%--<ext:Button ID="BtnBeforeExport" runat="Server" Scale="Medium"  Text="<%$ Resources:StringDef, BeforeCombineAccount %>" Width="100">
								<Listeners>
									<Click Fn="btnBeforeExport" />
								</Listeners>
						    </ext:Button>--%>
							...
							<%--<ext:Button ID="BtnAfterCombine" runat="Server" Scale="Medium" Text="<%$ Resources:StringDef, AfterCombineAccount %>" Width="100">
								<Listeners>
									<Click Fn="btnAfterCombine" />
								</Listeners>
							</ext:Button>--%>
						</Buttons>
					</ext:Panel>
				...
				<li>
					<h3><asp:Literal runat="server" Text="<%$ Resources:StringDef, MsgSvrCombineStep8 %>"></asp:Literal></h3>
					<ext:Panel ID="Panel10" runat="server" Layout="Form" Border="false" Width="750" LabelWidth="1" ButtonAlign="Center">
						<Items>
							<%--<ext:DisplayField ID="SvrWarOnCloseNote" runat="Server"  Text="<%$ Resources:StringDef, MsgSvrWarOnCloseNote %>" StyleSpec="font-weight:bold;color:red;">
							</ext:DisplayField>--%>
							<ext:DisplayField ID="SvrWarOnClose" runat="Server" Hidden="true"></ext:DisplayField>
						</Items>
						<Buttons>
							<%--<ext:Button ID="BtnCloseSvrWarOn" runat="Server" Scale="Medium" Text="<%$ Resources:StringDef,SvrWarOnCloseSwitch %>" Width="100">
								<Listeners>
									<Click Fn="svrWarOnKeyBoard" />
								</Listeners>
							</ext:Button>--%>
							<ext:Button runat="server" ID="Button10" Scale="Medium" Text="<%$ Resources:StringDef, FixWarSvrData %>" Width="100">
								<Listeners>
									<Click Fn="fixWarSvrData" />
								</Listeners>
							</ext:Button>
						</Buttons>
					</ext:Panel>
				...
				<li>
					<h3><asp:Literal ID="Literal3" runat="server" Text="<%$ Resources:StringDef, MsgSvrCombineStep9 %>"></asp:Literal></h3>
				...
				<li>
					<h3><asp:Literal ID="Literal4" runat="server" Text="<%$ Resources:StringDef, MsgSvrCombineStep10 %>"></asp:Literal></h3>
				...
				<li>
					<h3><asp:Literal runat="server" Text="<%$ Resources:StringDef, MsgSvrCombineStep11 %>"></asp:Literal></h3>
				...
				<li>
					<h3><asp:Literal runat="Server" Text="<%$ Resources:StringDef, MsgSvrCombineStep12 %>"></asp:Literal></h3>
					<ext:Panel ID="Panel20" runat="Server" Layout="Form" Border="false" Width="750" LabelWidth="1" ButtonAlign="Center">
						<Items>
							<ext:DisplayField ID="DisplayFieldRestartWarServerNote" runat="Server" Text="<%$ Resources:StringDef, MsgSvrCombineRestartWarServerNote %>" StyleSpec="font-weight:bold;color:red;">
							</ext:DisplayField>
						</Items>
					</ext:Panel>
					<br />
