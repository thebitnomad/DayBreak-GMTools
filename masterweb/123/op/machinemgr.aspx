...
//------------------------------- render func -----------------------------------------------------
			...
			var prepareMachineToolbar = function( grid, toolbar, rowIndex, record ) {
				...
				var btnProcCfg	= toolbar.items.get( 2);
				var btnFileList	= toolbar.items.get( 3 );
			...
			var showMachineTip = function( ) {
				...
				switch( cellIndex )
				{
					...
					case 7:
						{
							staticTip = 1;
							this.body.dom.innerHTML = record.get( 'GameGroups' );
						}
						break;
					default:
						{
...
//---------------------------------- other ------------------------------------------------------
			...
			var refreshLodingSelectChange = function( ) {
				#{PagingMachine}.pageSize = parseInt( this.getValue( ) );
				var paramStart = 0;
				var paramLimit = #{PagingMachine}.pageSize;
				#{StoreMachine}.load( { params: { start: paramStart, limit: paramLimit } } );
			};
		...
		<Reader>
			<ext:JsonReader>
				<Fields>
					...
					<ext:RecordField Name="GameGroups" Mapping="GameGroups" Type="String" />
				</Fields>
			</ext:JsonReader>
		</Reader>
	</ext:Store>
					...
					<ext:GridPanel ID="GridPanelMachine" runat="Server" Border="false" StoreID="StoreMachine" AutoExpandColumn="Task" >
						...
						<ColumnModel>
							<Columns>
								...
								<ext:Column Header="<%$ Resources:StringDef, CpuLoad %>" DataIndex="CpuLoad" Width="100" Align="Center">
									<Renderer Fn="renderCpuLoad" />
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef, NetworkLoad %>" DataIndex="NetworkLoad" Width="100" Align="Center">
									<Renderer Fn="renderNetworkLoad" />
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef,SvrName %>" DataIndex="GameGroups" Width="100" Align="Center" />
								<ext:Column Header="<%$ Resources:StringDef, CurrentTask %>" DataIndex="Task" Width="100">
								...
								<ext:CommandColumn Width="310">
									<Commands>
										<ext:GridCommand CommandName="View"  Icon="ServerGo" >
											<ToolTip Text="<%$ Resources:StringDef, RealTimeInfo %>" />
										</ext:GridCommand>
										<ext:CommandSeparator />
										<ext:GridCommand CommandName="ProcessConfig"  Icon="Cog" >
											<ToolTip Text="<%$ Resources:StringDef, ProcessConfig %>"/>
										</ext:GridCommand>
										<ext:CommandSeparator />
										<ext:GridCommand CommandName="FileList"  Icon="Page" >
											<ToolTip Text="<%$ Resources:StringDef, FileList %>"/>
										</ext:GridCommand>
										<ext:CommandSeparator />
										<ext:GridCommand CommandName="ShellCommand"  Icon="ComputerConnect" >
											<ToolTip Text="<%$ Resources:StringDef, ShellCommand %>"/>
										</ext:GridCommand>
									</Commands>
						...
						<BottomBar>
							<ext:PagingToolbar ID="PagingMachine" runat="server" PageSize="20" StoreID="StoreMachine">
								<Items>
									<ext:ToolbarSeparator />
									<ext:Label ID="Label1" runat="server" Text="<%$ Resources:StringDef, PageSize %>" />
									<ext:ToolbarSpacer ID="ToolbarSpacer1" runat="server" Width="5" />
									<ext:ComboBox ID="ComboBox1" runat="server" Width="50" Editable="false" >
										<Items>
					...
					<ext:GridPanel ID="GridPanelGameProc" runat="server" StoreID="StoreGameProc" Border="false" AutoExpandColumn="Comment" >
						...
						<ColumnModel>
							<Columns>
								...
								<ext:Column Header="<%$ Resources:StringDef, GameAppPath %>" DataIndex="GameAppPath" Width="300">
								</ext:Column>
								<ext:Column Header="<%$ Resources:StringDef, Comment %>" DataIndex="Comment" >
								</ext:Column>
